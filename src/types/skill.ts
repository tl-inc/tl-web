...
// ============================================================================
// Lexicon Skill Metadata - V3 Schema (2025-01)
// ============================================================================

/**
 * V3 Lexicon Schema Design Principles:
 * - Comprehensive vocabulary learning data
 * - Suitable for junior high school students
 * - Includes pronunciation, inflections, examples, and common mistakes
 * - Generated by OpenAI GPT-5 via Response API
 */

// Example sentence with translation
export interface LexiconExampleSentence {
  sentence: string;          // English example sentence (max 120 chars)
  translation: string;       // Chinese translation
}

// Additional meaning with context
export interface LexiconAdditionalMeaning {
  definition: string;        // Chinese definition
  example: string;           // English example
  translation: string;       // Chinese translation
  context: string;           // Usage context (e.g., "education", "informal")
}

// Inflection (詞形變化)
export interface LexiconInflection {
  form: string;              // The inflected form (e.g., "vocabularies", "ran")
  type: string;              // Type: "plural", "past", "past_participle", "present_participle", etc.
  example: string;           // Example sentence
  translation: string;       // Chinese translation
}

// Synonym or antonym
export interface LexiconSynonymAntonym {
...
// Common mistake
export interface LexiconCommonMistake {
  mistake: string;           // Incorrect usage
  correction: string;        // Correct usage
  explanation: string;       // Why it's wrong and how to fix
}

// Complete Lexicon Metadata (V3)
export interface LexiconMetadataV3 {
  word: string;              // Word in base form
  pos: string;               // Part of speech: "n.", "v.", "adj.", "adv.", etc.
  pronunciation: string;     // IPA or KK phonetics
  definitions: string[];     // Primary Chinese definitions (usually 1-2)

  // Noun-specific (conditional)
  countability?: 'countable' | 'uncountable' | 'both';

  // Verb-specific (conditional)
  transitivity?: 'transitive' | 'intransitive' | 'both';

  // Optional fields
  inflections?: LexiconInflection[];              // Word forms (plural, tenses, etc.)
  example_sentences: LexiconExampleSentence[];    // 2-5 example sentences
  synonyms?: LexiconSynonymAntonym[];             // Synonyms with examples
  antonyms?: LexiconSynonymAntonym[];             // Antonyms with examples
  additional_meanings?: LexiconAdditionalMeaning[]; // Other meanings with context
  common_mistakes?: LexiconCommonMistake[];       // 0-3 common mistakes
  summary: string[];                              // 2-5 bullet points for learning summary
}

// V3 is the current supported schema
export type LexiconMetadata = LexiconMetadataV3;
